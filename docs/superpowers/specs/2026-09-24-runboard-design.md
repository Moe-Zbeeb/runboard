...
## Architecture
```
train.py --(HTTP on cluster LAN, or shared FS)--> runboard serve --(outbound cloudflared)--> browser
```

Units:
1. `runboard.storage` — on-disk layout `<root>/<project>/<run_id>/{meta.json, metrics.jsonl}`.
   Append-only JSONL rows `{"_step", "_time", <metric>: <float>}`. Incremental reads by byte offset.
2. `runboard.client` — `init(project, name, config)`, `log(dict, step)`, `finish()`.
   ...
   - `http`: POST to server; on failure keeps rows in memory and retries with backoff; unsent rows
     at exit are written to a spool file, uploadable later with `runboard sync`.
   - `file`: writes directly to a shared log directory via `storage`.
   Server discovery: args → env (`RUNBOARD_SERVER`, `RUNBOARD_TOKEN`, `RUNBOARD_DIR`) → `~/.runboard/server.json`
   (written by `runboard serve`; home dirs are usually shared across cluster nodes).
3. `runboard.server` — `ThreadingHTTPServer` bound to `0.0.0.0`.
   ...
   Auth: persistent random token (`~/.runboard/token`), accepted as `?token=`, `Authorization: Bearer`, or cookie.
   `GET /?token=` sets an HttpOnly cookie and redirects.
4. `runboard.tunnel` — finds or downloads `cloudflared` into `~/.cache/runboard/`, runs a quick tunnel,
...
6. CLI — `runboard serve [--dir] [--port] [--host] [--tunnel]`, `runboard ls`, `runboard sync <spool>`.
